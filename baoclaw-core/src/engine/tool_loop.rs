...
pub fn strip_base64_images(value: &Value) -> Value {
    match value {
        Value::String(s) => {
            ...
            if s.len() > 10_000 {
                if let Ok(parsed) = serde_json::from_str::<Value>(s) {
                    ...
                    return Value::String(
                        serde_json::to_string(&stripped).unwrap_or_else(|_| s.clone()),
                    );
                }
                ...
                if k == "data" {
                    if let Value::String(s) = v {
                        if s.len() > 1000 && (s.starts_with("iVBOR") || s.starts_with("/9j/")) {
                            new_map.insert(
                                k.clone(),
                                Value::String("[image: base64 data removed]".to_string()),
                            );
                            continue;
                        }
                    }
        ...
        Value::Array(arr) => Value::Array(arr.iter().map(strip_base64_images).collect()),
        _ => value.clone(),
    }
}
