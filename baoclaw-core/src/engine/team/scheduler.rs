...
impl DagNode {
    ...
    pub fn with_dependencies(mut self, dep_ids: &[&str]) -> Self {
        self.dependencies
            .extend(dep_ids.iter().map(|s| s.to_string()));
        self
    }
...
impl DagScheduler {
    ...
    pub fn execution_waves(&self) -> Result<Vec<ExecutionWave>, SchedulerError> {
        ...
        loop {
            let mut ready: Vec<String> = self
                .nodes
                .keys()
                .filter(|id| !completed.contains(*id) && in_degree.get(*id) == Some(&0))
                .cloned()
                .collect();
    ...
    pub fn start_node(&mut self, id: &str) -> Result<(), SchedulerError> {
        let node = self.nodes.get_mut(id).ok_or_else(|| SchedulerError {
        ...
        while let Some(node_id) = to_skip.pop_front() {
            if let Some(node) = self.nodes.get_mut(&node_id) {
                if node.status == NodeStatus::Pending || node.status == NodeStatus::Ready {
                    ...
                    node.metadata
                        .insert("skip_reason".to_string(), reason.to_string());
...
mod tests {
    ...
    fn test_dag_node_is_ready() {
        let node = DagNode::new("node-2", "Task").with_dependency("node-1");
    ...
    fn test_scheduler_duplicate_node() {
        ...
        scheduler
            .add_node(DagNode::new("node-1", "Task 1"))
            .unwrap();
    ...
    fn test_scheduler_missing_dependency() {
        ...
        scheduler
            .add_node(DagNode::new("node-2", "Task 2").with_dependency("node-1"))
            .unwrap();
    ...
    fn test_topological_sort_with_dependencies() {
        ...
        scheduler
            ...
            .unwrap();
        scheduler
            .add_node(DagNode::new("c", "Task C").with_dependency("a"))
            .unwrap();
        scheduler
            .add_node(DagNode::new("d", "Task D").with_dependencies(&["b", "c"]))
            .unwrap();
    ...
    fn test_priority_ordering() {
        ...
        scheduler
            .add_node(DagNode::new("a", "Task A").with_priority(1))
            .unwrap();
        scheduler
            .add_node(DagNode::new("b", "Task B").with_priority(3))
            .unwrap();
        scheduler
            .add_node(DagNode::new("c", "Task C").with_priority(2))
            .unwrap();
