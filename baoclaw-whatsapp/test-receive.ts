...
sock.ev.on('connection.update', (update: any) => {
  ...
  if (update.connection === 'open') {
    ...
    console.log('Send a message from another phone to this WhatsApp account.\n');
  }
...
sock.ev.on('messages.upsert', (m: any) => {
  console.log('\n📨 [messages.upsert] type:', m.type, 'count:', m.messages?.length);
...
sock.ev.on('messaging-history.set' as any, (data: any) => {
  console.log('[messaging-history.set] messages:', data?.messages?.length, 'isLatest:', data?.isLatest);
});
