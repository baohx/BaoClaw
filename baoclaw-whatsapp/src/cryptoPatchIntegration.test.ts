...
try {
  const baileysCrypto: any = await import('@whiskeysockets/baileys/lib/Utils/crypto.js');
  if (baileysCrypto.aesEncrypt && baileysCrypto.aesDecrypt) {
    ...
    assert('Baileys crypto util loadable', false, '(aesEncrypt/aesDecrypt not exported)');
  }
} catch (err: any) {
...
try {
  ...
  if (baileysCrypto.aesEncrypWithIV && baileysCrypto.aesDecryptWithIV) {
    ...
    assert('Baileys aesEncrypWithIV/aesDecryptWithIV round-trip', plain.equals(dec));
  } else {
