...
function formatItemList(emoji: string, title: string, items: string[], count: number): string {
...
function formatSearchResults(results: SearchResult[], query: string): string {
  ...
  for (const r of results) {
    ...
    out += `[${ts}] ${role}\n${r.snippet || r.context || ''}\n\n`;
    if (out.length > MAX_OUTPUT) { out += '…(更多结果已截断)'; break; }
  }
  return out;
}
...
function formatSpecRun(result: { task_id?: string; status: string; message?: string }): string {
  if (result.message) {
    return `🚀 *Spec 执行*\n\n${result.message}`;
  }
...
function formatStatus(daemonInfo: { pid: number; session_id: string; cwd: string } | null, ipcClient: IpcClient): string {
...
export function setDaemonInfo(info: { pid: number; session_id: string; cwd: string } | null): void {
  _daemonInfo = info;
}
...
const historyCommand: Command = {
  ...
  async handler(ctx) {
    ...
    return formatHistory(result.entries ?? result as unknown as HistoryEntry[], n);
  },
};
 ...
 */
export async function dispatchCommand(ctx: CommandContext): Promise<string | null> {
