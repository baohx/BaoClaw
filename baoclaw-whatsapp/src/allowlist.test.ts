...
describe("normalizeJid", () => {
  test("extracts phone from WhatsApp JID", () => {
    assert.strictEqual(
      normalizeJid("12025551234@s.whatsapp.net"),
      "+12025551234",
    );
  });
...
describe("isAllowed", () => {
  test("returns true for allowlisted number", () => {
    assert.strictEqual(
      isAllowed("+12025551234", ["+12025551234", "+447911123456"]),
      true,
    );
  });
  ...
  test("normalizes JID before checking", () => {
    assert.strictEqual(
      isAllowed("12025551234@s.whatsapp.net", ["+12025551234"]),
      true,
    );
  });
});
        ...
        (sender, allowlist) => {
          ...
          if (allowlist.length === 0) {
            assert.strictEqual(
              result,
              false,
              "empty allowlist should reject all",
            );
          } else if (allowlist.includes(sender)) {
            assert.strictEqual(
              result,
              true,
              `sender ${sender} should be allowed when in list`,
            );
          } else {
            assert.strictEqual(
              result,
              false,
              `sender ${sender} should be rejected when not in list`,
            );
          }
        },
      ),
