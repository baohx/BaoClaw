...
if (nativeAesWorks()) {
  console.log('[crypto-patch] Native AES is working; skipping Baileys/libsignal patches.');
  process.exit(0);
}

console.warn('[crypto-patch] Native AES is failing; applying Baileys/libsignal patches.');
const patchPackage = require.resolve('patch-package');
const result = spawnSync(process.execPath, [patchPackage], { stdio: 'inherit' });
