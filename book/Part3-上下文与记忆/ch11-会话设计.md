...
daemon 重启后，采用**摘要优先**的三层策略恢复历史（从 10 分钟降至 < 5 秒）：

```rust
let old_summary_obj = SessionMemory::load(&rid);
...
let messages = if has_summary {
    // Tier 1（最佳）：有预写摘要
    // 只加载摘要 + 最近 200 条 entries → 瞬时
    rebuild_messages_from_transcript_limited(
        &entries, 200, Some(&old_summary),
    )
} else if entry_count <= 400 {
    // Tier 2（小 session）：安全 rebuild 全部
    rebuild_messages_from_transcript(&entries)
} else {
    // Tier 3（兜底）：大 session 无摘要
    // 只取最后 200 条 + 插入警告消息
    let tail_entries = &entries[entry_count - 200..];
    let mut msgs = rebuild_messages_from_transcript(tail_entries);
    msgs.insert(0, warning_message(
        "Session resumed — N earlier entries omitted. \
         Summary will be generated this session."
    ));
    msgs
};

engine.set_messages(messages);
engine.seed_session_memory(&old_summary);  // 携带旧摘要到新 session
```

| 场景 | 加载方式 | 耗时 |
|------|----------|------|
| 有预写摘要 | 摘要 + 最近 200 条 | < 5 秒 |
| 小 session（≤400 条） | rebuild 全部 | 秒级 |
| 大 session 无摘要 | 只取最后 200 条 | < 10 秒 |
