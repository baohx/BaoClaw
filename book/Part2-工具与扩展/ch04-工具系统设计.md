...
BaoClaw 使用 Rust 的 trait 定义统一的工具接口（`baoclaw-core/src/tools/trait_def.rs`）：
...
| GlobTool | 文件路径模式匹配 |
| WebFetchTool | 获取网页内容 |
| WebSearchTool | 网页搜索（Brave Search API） |
| AgentTool | 子 Agent（支持深度策略） |
| EvolveTool | 自我进化：创建/改进 skill、导出训练数据 |
...
工具执行在 `baoclaw-core/src/tools/executor.rs` 中实现：
